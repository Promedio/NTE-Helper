	...
	class macro {
		...
		class RepeatKeystrokeSingle extends Hotkeys.macro.RepeatKeystrokeBase {
			...
			Call(key_name_from_call) {
				...
				; 由于没有重叠的键，停止按键重复是由抬起按键所绑定的逻辑全权管理的，为防止客观疏漏，此处也有替代管理机制。
				this.press_key(true)
			} ; func Call
			...
			press_key(is_first_keystroke := false) {
				; 此处为防止因抬起热键的热键变体失去焦点而未能触发中止所带来的疏漏，此处触发则意味着宏相当于被触发了抬起，
				; 但是，热键是可能在保持按时下失去了焦点再回归释放的，在当前单键中暂时不会引发问题。具体细节如下——
		...
		class RepeatKeystrokeMultiple extends Hotkeys.macro.RepeatKeystrokeBase {
			...
			Call(key_name_from_call) {
				...
				; 该宏通过一个简单的类内变量来指示当前实际使用的按键和控制是否应当过滤当前按键，而具体的击键的订阅是由抬起按键的事件管理的。
				...
				; 判断击键条件并视情况立即触发并订阅下次击键。这是为了立刻响应新的击键而不是等到下一次订阅再触发。
				; 停止按键重复是由抬起按键所绑定的逻辑全权管理的，为防止客观疏漏，此处也有替代管理机制。
				; 由于新按键会在当前按键还未释放时顶替成为当前按键，所以此处每次击键都会立即停止现存的击键订阅。
				this.press_key()
			} ; func Call
			...
			press_key() {
				...
				; 此宏普遍存在多个按键重叠触发的情况，抬起事件无法完全管理（换言之是有条件的管理）。
				SetTimer(this.press_key_bind, 0)
			...
			; 外部按键传入——
			; 告知宏实例相关的按键已经放开并需要重置宏的状态。
			; 如果传入的按键确实为当前按键，立即停止击键并重置宏的状态。由于有这样的执行条件，应注意此抬起事件无法完全管理重复击键的停止。
			; 注意：此函数不接受自然按键传入。
